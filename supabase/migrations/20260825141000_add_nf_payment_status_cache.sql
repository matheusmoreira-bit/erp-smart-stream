ALTER TABLE public.sap_nf_entrada_cache
  ADD COLUMN IF NOT EXISTS paid_to_date numeric(18,4);

DROP FUNCTION IF EXISTS public.get_nf_entrada_cache_by_po(text, integer);

CREATE FUNCTION public.get_nf_entrada_cache_by_po(_company_db text, _po_doc_entry integer)
RETURNS TABLE(
  doc_entry integer,
  doc_num integer,
  series integer,
