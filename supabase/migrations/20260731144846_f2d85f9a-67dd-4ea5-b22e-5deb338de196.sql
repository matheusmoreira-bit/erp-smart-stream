CREATE INDEX IF NOT EXISTS idx_audit_trail_row_pk_id ON public.audit_trail (((row_pk->>'id')));
...
BEGIN
  ...
  SELECT n.created_at, 'notificacao', 'notification',
   ...
   WHERE n.created_at >= win_from
     AND (coalesce(n.metadata::text, '') LIKE '%' || k || '%'
       OR coalesce(n.link, '') LIKE '%' || k || '%')
  ...
  SELECT m.created_at, 'email', 'email',
   ...
   WHERE m.created_at >= win_from
     AND coalesce(m.metadata::text, '') LIKE '%' || k || '%'
