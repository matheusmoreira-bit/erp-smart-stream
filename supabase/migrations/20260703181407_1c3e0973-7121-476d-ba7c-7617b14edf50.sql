-- Fix audit trigger: text::bytea falha quando o payload contém '\'
-- (ex.: descrição/observação da NF com barra invertida). Usar convert_to.
...
BEGIN
  ...
  -- convert_to aceita qualquer texto UTF-8 (inclusive '\'), diferente de text::bytea
...
CREATE OR REPLACE FUNCTION public.verify_audit_chain(_limit integer DEFAULT NULL::integer)
...
BEGIN
  ...
  LOOP
    ...
    v_expected := extensions.digest(convert_to(v_payload, 'UTF8'), 'sha256'::text);
