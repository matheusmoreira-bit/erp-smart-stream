...
//
// Cron incremental: para cada empresa com credenciais SAP configuradas,
// consulta o Service Layer em /PurchaseInvoices filtrando por UpdateDate >
// cursor salvo em sap_nf_entrada_sync_state e faz upsert em
// sap_nf_entrada_cache. Cada NF traz o DocEntry do Pedido de Compra base
// (quando existir), o que facilita amarrar NF ↔ PC ↔ Contas a Pagar.
...
function toIsoTimestamp(date?: string, time?: string): string | null {
  if (!date) return null;
  // SAP UpdateTime pode vir como "HH:MM:SS" ou número (ex.: 1035)
  let t = "00:00:00";
  if (time) {
    if (/^\d{1,4}$/.test(time)) {
...
async function syncCompany(
  ...
  if (isTestCompanyDb(companyDb)) {
    return { companyDb, synced: 0, skipped: "test_base" };
  }
  ...
  const { data: stateRow } = await sb
    .from("sap_nf_entrada_sync_state")
  ...
  try {
    for (let page = 0; page < MAX_PAGES_PER_COMPANY; page++) {
      // Filtro incremental: por UpdateDate (data) ou fallback por DocEntry.
      // Usamos DocEntry como tie-breaker crescente.
      const filterParts: string[] = [];
      if (cursorUpdate) {
        const d = cursorUpdate.slice(0, 10);
        filterParts.push(`UpdateDate ge '${d}'`);
      }
      if (cursorEntry) {
        filterParts.push(`DocEntry gt ${cursorEntry}`);
      }
      const filter = filterParts.length ? `&$filter=${encodeURIComponent(filterParts.join(" and "))}` : "";
      const select = "$select=DocEntry,DocNum,Series,CardCode,CardName,DocDate,DocDueDate,TaxDate,DocTotal,DocCurrency,DocumentStatus,Cancelled,UpdateDate,UpdateTime,DocumentLines";
      const url = `${baseUrl}/PurchaseInvoices?${select}&$orderby=DocEntry asc&$top=${PAGE_SIZE}${filter}`;

      const r = await sapFetch(url, { headers: { Cookie: cookie, Prefer: "odata.maxpagesize=" + PAGE_SIZE } });
      if (!r.ok) {
        lastError = `PurchaseInvoices ${r.status}: ${(await r.text()).slice(0, 200)}`;
        break;
      }
      const j = await r.json();
      const items: SapPurchaseInvoice[] = j.value || [];
      ...
      if (upErr) {
        lastError = `upsert: ${upErr.message}`;
        break;
      }
  ...
  await sb.from("sap_nf_entrada_sync_state").upsert({
    company_db: companyDb,
...
Deno.serve(async (req) => {
  ...
  const gotLock = await tryWatcherLock(sb, "sap-nf-entrada-sync", 10);
  if (!gotLock) {
    return new Response(
  ...
  try {
    // Empresas com SAP habilitado: pega DISTINCT company_db da system_credentials
    ...
    for (const companyDb of companyDbs) {
      ...
      try {
        const res = await syncCompany(sb, companyDb);
        results.push(res);
      } catch (e) {
    ...
    await releaseWatcherLock(sb, "sap-nf-entrada-sync", "ok", `synced=${totalSynced} companies=${companyDbs.length}`);
    return new Response(JSON.stringify({ ok: true, total_synced: totalSynced, results }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
    await releaseWatcherLock(sb, "sap-nf-entrada-sync", "error", (e as Error).message);
