...
// public.sap_vendor_payment_cache. Salva PaymentInvoices em `invoice_links`
// para permitir cruzar Pagamento ↔ NF sem novas consultas ao SAP.
...
async function syncCompany(
  ...
  const { data: stateRow } = await sb
    .from("sap_vendor_payment_sync_state")
  ...
  try {
    for (let page = 0; page < MAX_PAGES_PER_COMPANY; page++) {
      ...
      const select = "$select=DocEntry,DocNum,Series,CardCode,CardName,DocDate,DocTotal,DocTotalFc,DocCurrency,DocumentStatus,Cancelled,UpdateDate,UpdateTime,PaymentInvoices";
      const url = `${baseUrl}/VendorPayments?${select}&$orderby=DocEntry asc&$top=${PAGE_SIZE}${filter}`;

      const r = await sapFetch(url, { headers: { Cookie: cookie, Prefer: "odata.maxpagesize=" + PAGE_SIZE } });
      if (!r.ok) { lastError = `VendorPayments ${r.status}: ${(await r.text()).slice(0, 200)}`; break; }
      const j = await r.json();
      const items: SapVendorPayment[] = j.value || [];
  ...
  await sb.from("sap_vendor_payment_sync_state").upsert({
    company_db: companyDb,
...
Deno.serve(async (req) => {
  ...
  const gotLock = await tryWatcherLock(sb, "sap-vendor-payment-cache-sync", 10);
  if (!gotLock) {
  ...
  try {
    ...
    await releaseWatcherLock(sb, "sap-vendor-payment-cache-sync", "ok", `synced=${totalSynced} companies=${companyDbs.length}`);
    return new Response(JSON.stringify({ ok: true, total_synced: totalSynced, results }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e) {
    await releaseWatcherLock(sb, "sap-vendor-payment-cache-sync", "error", (e as Error).message);
