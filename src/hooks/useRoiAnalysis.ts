...
export function useRoiAnalysis(opts: Options) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const buildExpenses = () => {
        let q = expenseRead("expenses").viewAll()
      ...
      // Descarta expenses anteriores para não contaminar a segregação SAP puro vs via Flow.
      // (approval_history vem do SAP e reflete o cenário nativo — mantemos todo o histórico.)
  ...
  }, [params, expenses, approvals, pos, fluxo, companyDb, consolidated, companies, from?.getTime(), to?.getTime()]);
