...
const ACTION_COLORS: Record<string, string> = {
  ...
  save: "bg-emerald-500/15 text-emerald-400 border-emerald-500/30",
...
export default function AuditLogTable({ entries, isLoading, showCompanyColumn = false }: AuditLogTableProps) {
  ...
  const filtered = useMemo(() => {
    ...
    if (entityFilter !== "all") list = list.filter((e) => e.entity_type === entityFilter);
    ...
    if (search.trim()) {
      ...
      list = list.filter(
        (e) =>
          ...
          e.company_db?.toLowerCase().includes(q) ||
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Select value={entityFilter} onValueChange={setEntityFilter}>
          <SelectTrigger className="w-[180px] bg-card"><SelectValue placeholder="Entidade" /></SelectTrigger>
          <SelectContent>
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-card z-10">
                <tr className="border-b border-border text-xs uppercase tracking-wider text-muted-foreground">
                  ...
                  {showCompanyColumn && <th className="px-4 py-3 text-left">Empresa</th>}
                ...
                {filtered.map((e) => (
                  <tr key={e.id} className="border-b border-border last:border-b-0 hover:bg-muted/20 transition-colors">
                    <td className="px-4 py-2.5 text-muted-foreground whitespace-nowrap">
                      <span className="flex items-center gap-1"><Clock className="w-3 h-3" />{formatDateTime(e.created_at)}</span>
                    </td>
                    <td className="px-4 py-2.5 font-medium text-foreground truncate max-w-[180px]">{e.actor_email || "—"}</td>
                    <td className="px-4 py-2.5">
                      <Badge variant="outline" className={getActionColor(e.action)}>{e.action}</Badge>
                    </td>
                    <td className="px-4 py-2.5 text-muted-foreground">{e.entity_type}</td>
                    {showCompanyColumn && <td className="px-4 py-2.5 text-muted-foreground font-mono text-xs">{e.company_db || "—"}</td>}
                    <td className="px-4 py-2.5 text-muted-foreground font-mono text-xs truncate max-w-[120px]">{e.entity_id || "—"}</td>
                    <td className="px-4 py-2.5 text-muted-foreground text-xs truncate max-w-[250px]">{e.details ? JSON.stringify(e.details) : "—"}</td>
                  </tr>
                ))}
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={showCompanyColumn ? 7 : 6} className="text-center text-muted-foreground py-12">
